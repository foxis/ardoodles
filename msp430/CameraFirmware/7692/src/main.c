...
unsigned char ov7692_wr (unsigned short regaddr, unsigned char data);
...
void main (void)
{
    ...
    ov7692_wr (0x12, 0x80); // RESET SENSOR
	ov7692_wr (0x0E, 0x08); // SLEEP
	pause (ms1);
    ...
    for (i = 0; i < sizeof(ov7692_init_settings_array)/sizeof(ov7692_init_settings_array[0]); i++)
	{
	    ov7692_wr (ov7692_init_settings_array[i].reg_addr, ov7692_init_settings_array[i].reg_val);
	}
